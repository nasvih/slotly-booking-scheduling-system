...
/* Dates follow whichever language is painted on <html>: Arabic month and
   weekday names, but Latin digits, so a calendar column stays the width the
   grid was built for and a time still reads 09:30. Nothing here decides the
   first day of the week — the app works that out from getDay() and the locale
   is never allowed to reorder the grid. */
export const dateLocale = () => (document.documentElement.lang === 'ar' ? 'ar-u-nu-latn' : 'en-GB');

/* "3h ago" belongs to the app's dictionary, not to this shared runtime.
   The app registers its four forms at boot; until it does, English stands. */
let REL = {
  justNow: () => 'just now',
  minutes: (n) => `${n}m ago`,
  hours: (n) => `${n}h ago`,
...
export const setRelativeLabels = (map) => { REL = Object.assign({}, REL, map); };
...
export function ago(d) {
  ...
  if (s < 60) return REL.justNow();
  if (s < 3600) return REL.minutes(Math.floor(s / 60));
  if (s < 86400) return REL.hours(Math.floor(s / 3600));
  if (s < 2592000) return REL.days(Math.floor(s / 86400));
  return fmtDate(d);
}
...
export function confirmDialog(message, { title = 'Confirm', danger = false, okLabel = 'Confirm', cancelLabel = 'Cancel' } = {}) {
