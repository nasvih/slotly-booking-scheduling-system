...
/* Dates follow the language written onto <html> before the first paint.
   Arabic gets Arabic month names but Latin digits (`-u-nu-latn`), because a
   table column of dates has to line up with the numbers in the columns
   either side of it — Eastern Arabic numerals in one column and Latin in
   the next reads as two different tables. */
export const dateLocale = () => (document.documentElement.lang === 'ar' ? 'ar-u-nu-latn' : 'en-GB');

/* This file is shared by the demo apps and holds no strings of its own, so
   the few sentences it says are injected from the app's dictionary. The
   English defaults below are what shows until an app hands its own over, and
   stand for good in an app that never does.
   `setAgoStrings` covers relative time; `setUiStrings` covers the words the
   runtime renders on its own — the two dialog buttons and the modal close. */
let AGO = {
  justNow: 'just now',
  m: (n) => `${n}m ago`,
  h: (n) => `${n}h ago`,
...
export function fmtTime(d) {
  const dt = d instanceof Date ? d : new Date(d);
  return dt.toLocaleTimeString(dateLocale(), { hour: '2-digit', minute: '2-digit', hour12: false });
}
export function ago(d) {
  ...
  if (s < 2592000) return AGO.d(Math.floor(s / 86400));
  return fmtDate(d);
}
...
export function confirmDialog(message, { title = UI.confirm, danger = false, okLabel = UI.confirm, cancelLabel = UI.cancel } = {}) {
  return new Promise((resolve) => {
    modal({
      title,
      body: `<p class="muted">${esc(message)}</p>`,
      actions: [
        { label: cancelLabel, class: '', onClick: () => resolve(false) },
