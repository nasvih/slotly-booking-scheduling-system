...
function tableHTML(t) {
  ...
  /* the wrapper is the scroll container: a bubble in a 390px panel cannot
     shrink below a four column table, so the table keeps its natural width
     and scrolls inside the message instead of pushing the bubble out. */
...
export class Assistant {
  constructor(cfg) {
    this.cfg = Object.assign({
      ...
      context: () => ({}),
      /* Chrome wording. This file is shared by the demo apps and carries no
         dictionary of its own, so the app passes its own translations in and
         the English below is what shows until it does. */
      ui: {},
    }, cfg);
    /* The merged bag, kept apart from `cfg` so `cfg.ui` stays exactly what the
       app passed. The two labels that name the assistant are functions: only
       the caller knows where the name belongs in its own word order. */
  ...
  mount(root = document.body) {
    ...
    fab.setAttribute('aria-label', this.ui.openAria(this.cfg.name));
    ...
    host.innerHTML = `
      <section class="assist${docked ? ' assist--docked' : ''}" role="dialog" aria-label="${esc(this.cfg.name)}">
        ...
        <form class="assist__form" data-form>
          <input class="input" placeholder="${esc(this.ui.placeholder)}" aria-label="${esc(this.ui.placeholder)}" autocomplete="off" data-input>
  ...
  async ask(q) {
    ...
    catch (err) { out = { text: this.ui.edge }; }
    ...
    if (trace) thinking.innerHTML += `<div class="msg__meta">${esc(trace)} · ${esc(this.ui.seconds((0.3 + Math.random() * 1.4).toFixed(2)))}</div>`;
  ...
  _renderActions(body, actions) {
    ...
    actions.forEach((a, i) => {
      ...
      btn.addEventListener('click', async () => {
        ...
        let result;
        try { result = await a.run(); } catch (err) { result = { text: this.ui.failed }; }
