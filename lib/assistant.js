...
export class Assistant {
  constructor(cfg) {
    ...
    /* The panel's own furniture. An app that ships a dictionary passes its
       translations in as `ui`; anything it leaves out keeps the English here,
       so an app that passes nothing is unchanged. */
  ...
  mount(root = document.body) {
    ...
    fab.type = 'button';
    fab.title = this.ui.askAbout(this.cfg.name);
    ...
    host.innerHTML = `
      <section class="assist${docked ? ' assist--docked' : ''}" role="dialog" aria-label="${esc(this.cfg.name)}">
        <header class="assist__head">
          ...
          <button class="btn btn--ghost btn--icon" data-act="reset" title="${esc(this.ui.clear)}">${ICON.reset}</button>
          ${docked ? '' : `<button class="btn btn--ghost btn--icon" data-act="close" title="${esc(this.ui.close)}">${ICON.close}</button>`}
        </header>
        ...
        <form class="assist__form" data-form>
          ...
          <button class="btn btn--primary btn--icon" type="submit" title="${esc(this.ui.send)}">${ICON.send}</button>
        </form>
  ...
  async ask(q) {
    ...
    catch (err) { out = { text: this.ui.edgeCase }; }
  ...
  _renderActions(body, actions) {
    ...
    actions.forEach((a, i) => {
      ...
      btn.addEventListener('click', async () => {
        ...
        let result;
        try { result = await a.run(); } catch (err) { result = { text: this.ui.actionFailed }; }
