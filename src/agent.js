...
function matchDay(q) {
  ...
  if (/\btomorrow\b/.test(text)) return addDays(t, 1);
...
function matchService(state, q) {
  const text = q.toLowerCase();
  const hints = [
    [/\bdental clean|\bcleaning\b|\bscaling\b/, 'svc-clean'],
  ...
  for (const [re, id] of hints) {
    if (re.test(text) && state.services.some((s) => s.id === id && s.active)) return id;
  }
  return null;
}

function matchStaff(state, q) {
  const text = q.toLowerCase();
...
function matchCustomer(state, q) {
  const text = q.toLowerCase();
  let best = null;
  for (const c of state.customers) {
    const first = c.name.split(' ')[0].toLowerCase();
    const last = c.name.split(' ')[1] ? c.name.split(' ')[1].toLowerCase() : '';
...
export function buildAgent(ctx) {
  return new Assistant({
    ...
    greeting: 'I read the live rota in this browser. Ask me what is free, how loaded a day is, who is likely to miss an appointment, or when a named staff member is next open.',
    suggestions: ['Free slots tomorrow', "Today's load", 'Busiest hour', 'Who might not turn up'],
    ...
    intents: intents(),
    fallbacks: [
      'I can only answer from this desk\'s own data. Try: "what is free on Friday for physiotherapy".',
      'Not something I hold. Ask me about the queue, the rota, no-show rates or takings.',
      'I did not catch a date, a service or a name in that. Ask me which slots are free on Friday and I will pull the rota.',
      'That one is outside the demo data set. Try "next available with Anwar" or "busiest hour".',
    ],
    context: () => ctx.state,
  });
}
