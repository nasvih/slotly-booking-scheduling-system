...
import {
  ...
  CHANNELS, channelLabel, svcName, staffRole,
} from './data.js';
...
function slotPicker(state, { date, serviceId, staffId, ignoreId, selected, onPick }) {
  ...
  if (!slots.length) {
    return h('div', { class: 'empty', style: 'padding:20px' },
      h('h3', {}, t(isOpenDay(state, date) ? 'dialog.noSlotHead' : 'dialog.closedHead')),
      h('p', { class: 'small muted' }, t(isOpenDay(state, date) ? 'dialog.noSlotBody' : 'dialog.closedBody')));
  }
  slots.forEach((s) => {
...
function customerField(state, chosen) {
  const sel = h('select', { class: 'select', name: 'customer', 'aria-label': t('dialog.customer') },
    h('option', { value: '__new' }, t('dialog.newCustomer')),
    state.customers.slice().sort((a, b) => a.name.localeCompare(b.name))
      .map((c) => h('option', { value: c.id, selected: c.id === chosen }, t('dialog.customerOption', { name: c.name, phone: c.phone }))));
  return sel;
}
...
export function openBooking(ctx, prefill = {}) {
  ...
  if (!active.length) { toast(t('dialog.noService'), 'bad'); return; }
  ...
  const staffSel = h('select', { class: 'select', name: 'staff', 'aria-label': t('dialog.staffAria') });
  const svcSel = h('select', { class: 'select', name: 'service', 'aria-label': t('dialog.service') });
  const dateIn = h('input', { class: 'input', type: 'date', value: draft.date, min: todayKey(), 'aria-label': t('dialog.date') });
  const custSel = customerField(state, draft.customerId);
  const newName = h('input', { class: 'input', placeholder: t('dialog.fullName') });
  const newPhone = h('input', { class: 'input', dir: 'ltr', placeholder: t('dialog.phonePlaceholder') });
  const newWrap = h('div', { class: 'fieldrow', style: 'margin-top:12px' },
    h('label', { class: 'field' }, h('span', { class: 'field__label' }, t('dialog.name')), newName),
    h('label', { class: 'field' }, h('span', { class: 'field__label' }, t('dialog.phone')), newPhone));
  newWrap.hidden = true;
  const noteIn = h('input', { class: 'input', placeholder: t('dialog.notePlaceholder') });
  /* The value stored on the record stays the English key; the option shows the
     reader's word for it. */
  const channelSel = h('select', { class: 'select', 'aria-label': t('dialog.channel') },
    ['Front desk', 'Phone', 'Website', 'Walk-in'].map((c) => h('option', { value: c }, channelLabel(c))));
  ...
  const fillServices = () => {
    ...
    }, t('dialog.serviceOption', { name: svcName(svcOf(state, s.id)), duration: s.durationMin, price: `${state.settings.currency}${s.priceInr}` }))));
  };
  const fillStaff = () => {
    const eligible = state.staff.filter((s) => s.active && s.skills.includes(draft.serviceId));
    staffSel.innerHTML = '';
    staffSel.appendChild(h('option', { value: ANY, selected: draft.staffId === ANY }, t('dialog.anyAvailable')));
    eligible.forEach((s) => staffSel.appendChild(h('option', {
      value: s.id, selected: s.id === draft.staffId,
    }, t('dialog.staffOption', { name: s.name, role: staffRole(staffOf(state, s.id)) }))));
  ...
  const updateSummary = () => {
    ...
    if (!draft.time) { summary.textContent = t('dialog.pickTime'); return; }
    ...
    const who = ids.length ? staffOf(state, ids[0]).name : t('dialog.nobodyFree');
    summary.textContent = t('dialog.summary', {
      day: relativeDay(draft.date), time: hm12(draft.time),
      duration: svc.durationMin, buffer: svc.bufferMin, who,
    });
  };
  ...
  const body = h('div', {},
    h('div', { class: 'fieldrow' },
      h('label', { class: 'field' }, h('span', { class: 'field__label' }, t('dialog.service')), svcSel),
      h('label', { class: 'field' }, h('span', { class: 'field__label' }, t('dialog.staff')), staffSel)),
    h('div', { class: 'fieldrow', style: 'margin-top:14px' },
      h('label', { class: 'field' }, h('span', { class: 'field__label' }, t('dialog.date')), dateIn),
      h('label', { class: 'field' }, h('span', { class: 'field__label' }, t('dialog.channel')), channelSel)),
    h('div', { class: 'field', style: 'margin-top:14px' },
      h('span', { class: 'field__label' }, t('dialog.openSlots')), slotHost, summary),
    h('label', { class: 'field', style: 'margin-top:14px' },
      h('span', { class: 'field__label' }, t('dialog.customer')), custSel),
    newWrap,
    h('label', { class: 'field', style: 'margin-top:14px' },
      h('span', { class: 'field__label' }, t('dialog.note')), noteIn));

  refreshSlots();

  const dlg = modal({
    title: prefill.title || t('dialog.title'),
    width: '560px',
    body,
    actions: [
      { label: t('common.cancel') },
      {
        label: prefill.okLabel || t('dialog.ok'),
        class: 'btn--primary',
        onClick: () => {
          if (!draft.time) { toast(t('dialog.needTime'), 'bad'); return true; }
          let customerId = custSel.value;
          if (customerId === '__new') {
            const name = newName.value.trim();
            if (!name) { toast(t('dialog.needName'), 'bad'); return true; }
          ...
          const ids = freeStaffAt(ctx.state, draft.date, draft.time, draft.serviceId, prefill.ignoreId)
            .filter((id) => draft.staffId === ANY || id === draft.staffId);
          if (!ids.length) { toast(t('dialog.justFilled'), 'bad'); refreshSlots(); return true; }
          ...
          toast(t('dialog.booked', {
            token: label, name: custOf(ctx.state, customerId).name,
            day: relativeDay(draft.date), time: hm12(draft.time),
          }), 'ok');
          ...
          if (fresh) openToken(ctx, fresh, { title: prefill.tokenTitle || t('dialog.bookedTitle') });
          return false;
        },
      },
    ],
...
export function openReschedule(ctx, booking) {
  const state = ctx.state;
  openBooking(ctx, {
    title: t('dialog.rescheduleTitle', { ref: booking.ref }),
    okLabel: t('dialog.moveOk'),
    tokenTitle: t('dialog.movedTitle'),
    date: booking.date,
    ...
    onDone: (made) => {
      ctx.store.update((s) => {
        ...
        logEvent(fresh, 'moved', {
          fromDay: dayLabel(old.date), fromTime: hm12(old.time),
          toDay: dayLabel(fresh.date), toTime: hm12(fresh.time),
        });
        s.bookings = s.bookings.filter((b) => b.id !== old.id);
        assignTokens(s.bookings);
      });
      toast(t('dialog.moved', { ref: booking.ref }), 'ok');
    },
  });
  void state;
}
...
export async function cancelBooking(ctx, booking) {
  ...
  const ok = await confirmDialog(
    t('dialog.cancelBody', { ref: booking.ref, name, day: dayLabel(booking.date), time: hm12(booking.time) }),
    { title: t('dialog.cancelTitle'), danger: true, okLabel: t('dialog.cancelOk'), cancelLabel: t('common.close') });
  if (!ok) return false;
  ctx.store.update((s) => {
    const b = s.bookings.find((x) => x.id === booking.id);
    if (!b) return;
    b.status = 'cancelled';
    logEvent(b, 'cancelledAtDesk');
  });
  toast(t('dialog.cancelled', { ref: booking.ref }), '');
  return true;
}

export function setStatus(ctx, booking, status, key) {
  ctx.store.update((s) => {
    const b = s.bookings.find((x) => x.id === booking.id);
    if (!b) return;
    b.status = status;
    if (key) logEvent(b, key);
    else logEvent(b, 'marked', { status });
  });
}
